...
Part of the [telemetry gear](../).

## Install

```sh
npm install @gears-web/telemetry
```

ESM only. Requires a browser environment (`window`, `document`, `localStorage`, `fetch`).

## Usage

```ts
import { createTelemetry } from '@gears-web/telemetry';
...
```ts
import { createTelemetry, telemetryLocalePlugin } from '@gears-web/telemetry';
...
```ts
import { telemetryElementHookKey } from '@gears-web/telemetry';
...
- `destroy()` does not fully remove autocapture's capture-phase listeners.
- Several `context_*` record fields are declared but never populated.

## Development

```sh
nvm use          # 24.13.0, per .nvmrc
pnpm install
pnpm lint        # eslint + tsc on both tsconfigs
pnpm test
pnpm build
```

`engines` declares `node >=20.19`, which is the floor the toolchain actually needs — Vite requires
`^20.19.0 || >=22.12.0`. Develop on the `.nvmrc` version.
