...
export default defineConfig({
  plugins: [collector()],
  optimizeDeps: {
    // The SDK is a linked directory dependency. Vite would pre-bundle it and keep serving that
    // cached copy after `pnpm --dir ../../telemetry build` replaces dist/, so the page silently
    // runs stale SDK code. Excluding it means the demo always executes the current build.
    exclude: ['@gears-web/telemetry'],
  },
  server: {
    // Pinned so the `url` in src/main.ts stays correct.
    port: 5173,
    strictPort: true,
  },
});
